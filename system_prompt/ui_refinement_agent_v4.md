...
- **正确流程**：
  ...
  4. **🚨 永远优先找出明显的差异**：
     ...
     - **每轮迭代都要优先找出最明显的差异，不能假设"这轮应该关注细节"而忽略明显的差异**
  5. **逐像素、逐区域、逐元素**进行视觉检查
  6. **描述在图片中看到的具体差异**（如："target_image.png左上角按钮颜色是橙色，复刻页面截图相同位置按钮颜色是红色"）
- **必须找出至少 5 处最明显的不一样**（必须是从截图视觉对比中发现的实际差异，优先找出最明显的）
- **差异类型包括**（全部基于截图视觉观察）：
  - 颜色不一致（在截图中看到的背景色、文字颜色、边框颜色不同）
  - 尺寸不一致（在截图中看到的元素大小、宽度、高度不同）
  - 位置不一致（在截图中看到的元素位置、对齐方式、间距不同）
  - 布局不一致（在截图中看到的元素排列、顺序、方向不同）
  - 缺失元素（在截图中看到原网页有但复刻页面没有的元素）
...
- **每处差异必须描述在图片中看到的具体表现**（如："target_image.png显示背景色为浅灰色，复刻页面截图显示为白色"）
- **⚠️ 列出所有差异后，接下来的步骤5和6采用逐个处理方式**：
  - **处理顺序**：先从明显的差异开始，按照从上到下的顺序处理
  - 不需要一次性查看所有差异点的源码
  - 而是：查看差异点1（最明显/最上方） → 修复差异点1 → 查看差异点2 → 修复差异点2 → ... 逐个完成
...
**🚨 重要：永远优先修复明显的差异，不能按轮次来区分关注点！**

- **所有轮次都必须遵循以下优先级**（按明显程度排序）：
...
- ⚠️ **差异分析**：每轮必须从截图对比中找出至少 5 处最明显的不一样（全部基于截图视觉观察）
...
- [ ] 每次迭代必须找出至少 5 处最明显的不一样
- [ ] **🚨 差异必须100%从图片视觉对比中找出**：
  - ✅ 必须仔细对比 `target_image.png` 和复刻页面截图，逐像素、逐区域、逐元素进行视觉检查
  - ✅ 找出至少 5 处从图片中实际看到的不一致
  - ❌ 禁止查看源代码或CSS文件来找差异
  - ❌ 禁止通过代码对比来分析差异
  - ❌ 禁止基于假设或猜测来找差异
- [ ] **🚨 必须去原网页确认信息**：
...
- [ ] **每次修复后必须验证问题是否修复**
- [ ] **🚨 永远优先修复明显的差异**：所有轮次都必须优先找出并修复最明显的差异，不要因为轮次数高了就关注细节
...
- ⚠️ **每次截图后必须读取图片文件**（使用 `read_file`），仔细对比 `target_image.png` 和复刻页面截图
- 🚨 **差异必须100%从图片视觉对比中找出**：
  - ✅ 5 处差异必须通过仔细对比 `target_image.png` 和复刻页面截图找出，逐像素、逐区域、逐元素进行视觉检查
  - ✅ 差异必须是在图片中实际看到的不一致
  - ❌ 禁止查看源代码或CSS文件来找差异
  - ❌ 禁止通过代码对比来分析差异
  - ❌ 禁止基于假设或猜测来找差异
