...
- **🚨 重要：直接视觉对比完整截图，不要做差分图、热力图、像素差分等额外处理！**
- **禁止行为**：
  - ❌ **禁止查看源代码或CSS文件来找差异**
  - ❌ **禁止通过代码对比来分析差异**
  - ❌ **禁止基于"可能有问题"的假设来找差异**
  - ❌ **禁止凭空想象或猜测差异**
  - ❌ **禁止制作差分图、热力图、像素对比图等辅助工具**
- **正确流程**：
  1. **只使用步骤3中读取的两张完整图片**（`target_image.png` 和 `replica_page_iteration_N.png`）
  2. **直接通过视觉观察对比**两张完整截图找出差异，不需要任何额外的图像处理
...
- 🚨 **差异必须100%从图片视觉对比中获取**：
  - ✅ **正确做法**：直接视觉观察对比 `target_image.png` 和复刻页面截图的完整图片找出差异
  - ❌ **禁止做法**：查看源代码、CSS文件、代码对比来找差异
  - ❌ **禁止做法**：基于代码分析或"可能有问题"的假设来找差异
  - ❌ **禁止做法**：制作差分图、热力图、像素对比图等辅助工具
  - 5 处差异必须全部是**从完整截图视觉对比中实际看到的不一致**
- 🚨 **差异优先级**：
  - **第1优先级**：缺失元素、多余元素
...
- [ ] **🚨 差异必须100%从完整截图视觉对比中找出**：
  - ✅ 必须直接仔细对比 `target_image.png` 和复刻页面截图的完整图片，逐区域、逐元素进行视觉检查
  - ✅ 找出至少 5 处从完整图片中实际看到的不一致
  - ❌ 禁止查看源代码或CSS文件来找差异
  - ❌ 禁止通过代码对比来分析差异
  - ❌ 禁止基于假设或猜测来找差异
  - ❌ **禁止制作差分图、热力图、像素对比图等辅助工具**
...
- 🚨 **差异必须100%从完整截图视觉对比中找出**：
  - ✅ 5 处差异必须通过直接仔细对比 `target_image.png` 和复刻页面截图的完整图片找出，逐区域、逐元素进行视觉检查
  - ✅ 差异必须是在完整图片中实际看到的不一致
  - ❌ 禁止查看源代码或CSS文件来找差异
  - ❌ 禁止通过代码对比来分析差异
  - ❌ 禁止基于假设或猜测来找差异
  - ❌ **禁止制作差分图、热力图、像素对比图等任何辅助图像工具**
